...
asciinema bash-completion bashtop bat bc blesh-git cmatrix cowsay cron downgrade espeakup figlet-fonts file-roller fortune-mod git imagemagick jdk-openjdk jq lib32-glibc lolcat lsd myman nano-syntax-highlighting ncdu neofetch nvchad-git nyancat openbsd-netcat openvpn orca p7zip paru pfetch polkit powershell-bin python-pywhat reflector sl superbfetch-git textart tidy tk tmux toilet-fonts tree ufw unzip vim vnstat wget which xclip xmlstarlet zoxide \
openssl \
athena-neofetch-config athena-powershell-config athena-config athena-tmux-config athena-vim-config athena-zsh htb-toolkit nist-feed \
athena-alacritty-config athena-cyber-hub athena-firefox-config athena-gnome-config athena-kitty-config athena-samurai-theme athena-vscodium-themes athena-welcome bless cyberchef-electron gtk-engine-murrine hexedit networkmanager networkmanager-openvpn octopi onionshare torbrowser-launcher \
